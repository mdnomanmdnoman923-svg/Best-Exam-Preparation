// bep-full-project/src/features/profile/components/EducationLevelSelect.tsx
...
export default function EducationLevelSelect({
  levels,
  value,
  title = 'Education Level Select',
  subtitle = 'আপনার education stage select করুন',
  placeholder = 'Level search করুন...',
  searchable = true,
  loading = false,
  disabled = false,
  allowClear = true,
  ...
  const selectedLevel = useMemo(
    () => levels.find((item) => item.id === value) || null,
    [levels, value],
  );
  ...
  const filteredLevels = useMemo(() => {
    ...
    return levels.filter((item) =>
      [item.label, item.slug, item.description, item.curriculum, item.gradeRange]
        .filter(Boolean)
        .join(' ')
        .toLowerCase()
        .includes(query),
      ...
      <div className="relative overflow-hidden border-b border-white/10 p-6">
        ...
        <div className="relative z-10 flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Badge variant="premium">{levels.length} Levels</Badge>
        ...
        <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
          <div className="space-y-5">
            ...
            <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-5">
              <div className="mb-4 flex items-center gap-2 text-cyan-100">
                <GraduationCap className="h-4 w-4" />
                <span className="text-sm font-semibold">Selected level details</span>
              </div>
              ...
              {selectedLevel ? (
                <div className="space-y-4">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {selectedLevel.premium ? <Badge variant="premium">Premium</Badge> : null}
                    {selectedLevel.featured ? <Badge variant="secondary">Featured</Badge> : null}
                  </div>
                  ...
                  {selectedLevel.slug || selectedLevel.curriculum ? (
                  ...
                  {showStats && typeof selectedLevel.totalPrograms === 'number' ? (
                    ...
                    <div className="rounded-2xl border border-cyan-400/15 bg-cyan-400/10 p-4">
                      <p className="text-sm leading-7 text-white/75">
                        আপনার profile এ এই education level টি active আছে। আপনি চাইলে অন্য level select করতে পারেন।
                      </p>
                    </div>
                  )}
                ...
                <div className="flex min-h-[260px] flex-col items-center justify-center text-center">
                  ...
                  <h3 className="mt-5 text-xl font-semibold text-white">
                    কোনো level selected হয়নি
                  </h3>

                  <p className="mt-2 max-w-sm text-sm leading-7 text-white/55">
                    Right side থেকে আপনার education level choose করুন।
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="rounded-[28px] border border-white/10 bg-[#08111F]/70 p-5 backdrop-blur-xl">
            <div className="mb-4 flex items-center gap-2 text-fuchsia-100">
              <BookOpen className="h-4 w-4" />
              <span className="text-sm font-semibold">Available levels</span>
            </div>
            ...
            {loading ? (
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 text-sm text-white/55">
                            {[item.slug, item.curriculum].filter(Boolean).join(' • ') ||
                        ...
                        {showStats && typeof item.totalPrograms === 'number' ? (
              ...
              <div className="flex min-h-[260px] flex-col items-center justify-center text-center">
                ...
                <p className="mt-2 max-w-sm text-sm leading-7 text-white/55">
                  আপনার search অনুযায়ী কোনো level মেলেনি।
                </p>
              </div>
            )}
          </div>
